  ...
  const webpackConfig = {
    ...
    module: {
      rules: [
              ...
              name:
                function (pathname) {
                  if (webpackConfig.mode === 'development') {
                    ...
                    // 此处的调整会影响引用的调整
                    return path.relative(path.resolve(__dirname, 'src'), pathname)
                  }
                  // 调整输出路径 src/assets/media/dir/dir/image.jpg → assets/media/dir-dir-image.jpg
                  // 此处的调整会影响引用的调整
          ...
          use: {
            ...
            options: {
              ...
              name:
                function (pathname) {
                  // 将路径分隔符替换为 - 分隔符
                  return 'assets/' + path.relative(path.resolve(__dirname, 'src/assets/fonts'), pathname).split(path.sep).join('-')
                }
            }
          }
        },
        ...
        //     options: {
        //       loaders: {
        //         // 针对产生的css使用MiniCssExtractPlugin插件提取
        //         sass: MiniCssExtractPlugin.extract({
        //           // 注意：loader的参数可以通过JSON.stringify(OptionObject)这种形式传入
        //           use: 'css-loader!postcss-loader!sass-loader?' + JSON.stringify({includePaths: [path.resolve(__dirname, 'src')]}),
    ...
    plugins: [
      ...
      new MiniCssExtractPlugin({
        ...
        filename: '[name].css'
        // chunkFilename: function (data) {
        //   var name = (data.name + '.css').replace(/\.js.css$/, '.css').replace('/scripts/vendor.css', '/styles/vendor.css')
        //   name = name.replace('.css','.css.liquid')
        //   return name
        // }
      })
    ],
